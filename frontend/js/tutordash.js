    ...
    async function uploadContent() {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/courses/${course_id}/content`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    async function fetchCourses() {
      try {
        ...
        courses.filter(c => c.instructor_id === user.id).forEach(course => {
    ...
    async function fetchStudentProgress(studentId) {
      try {
        ...
        for (const course of instructorCourses) {
          const progressResponse = await fetch(`http://localhost:3000/api/courses/${course.id}/students`);
          const students = await progressResponse.json();
          const student = students.find(s => s.id === studentId);
          if (student) {
            progressHtml += `
                            <p><strong>${course.title}</strong>: ${student.progress}% complete</p>
                        `;
          }
        }
    ...
    async function fetchAnalytics() {
      try {
        ...
        courses.filter(c => c.instructor_id === user.id).forEach(async course => {
          const analyticsResponse = await fetch(`http://localhost:3000/api/courses/${course.id}/analytics`);
          ...
          li.innerHTML = `
                        <div>
                            ...
                            Average Rating: ${analytics.avg_rating.toFixed(1)}/5 (${analytics.review_count} reviews)<br>
    ...
    async function fetchEarnings() {
      try {
        const response = await fetch(`http://localhost:3000/api/instructors/${user.id}/earnings`);
        const data = await response.json();
